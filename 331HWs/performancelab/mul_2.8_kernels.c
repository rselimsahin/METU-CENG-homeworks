    	...
    	for(j=0;j<dim;j+=4){
            ...
            for(k=0;k<dim;k+=4){
                ...
                dst[(ix)*dim+(j)]+=src[(ix)*dim+(k)]*src2[(j)*dim+(k)]
                				 +src[(ix)*dim+(k+1)]*src2[(j)*dim+(k+1)]
                				 +src[(ix)*dim+(k+2)]*src2[(j)*dim+(k+2)]
                				 +src[(ix)*dim+(k+3)]*src2[(j)*dim+(k+3)];

                dst[(ix)*dim+(j+1)]+=src[(ix)*dim+(k)]*src2[(j+1)*dim+(k)]
                				   +src[(ix)*dim+(k+1)]*src2[(j+1)*dim+(k+1)]
                				   +src[(ix)*dim+(k+2)]*src2[(j+1)*dim+(k+2)]
                				   +src[(ix)*dim+(k+3)]*src2[(j+1)*dim+(k+3)];

                dst[(ix)*dim+(j+2)]+=src[(ix)*dim+(k)]*src2[(j+2)*dim+(k)]
                				   +src[(ix)*dim+(k+1)]*src2[(j+2)*dim+(k+1)]
                				   +src[(ix)*dim+(k+2)]*src2[(j+2)*dim+(k+2)]
                				   +src[(ix)*dim+(k+3)]*src2[(j+2)*dim+(k+3)];

                dst[(ix)*dim+(j+3)]+=src[(ix)*dim+(k)]*src2[(j+3)*dim+(k)]
                				   +src[(ix)*dim+(k+1)]*src2[(j+3)*dim+(k+1)]
                				   +src[(ix)*dim+(k+2)]*src2[(j+3)*dim+(k+2)]
                				   +src[(ix)*dim+(k+3)]*src2[(j+3)*dim+(k+3)];
                ix++;
